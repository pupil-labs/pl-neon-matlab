...
try
    ...
    % initialize detection of surface
    sc_sample = device.receive_scene_video_frame();
    gaze_mapper.process_scene(sc_sample);
    ...
    while true
        ...
        % Neon Scene Camera runs at 30Hz; by default
        if (GetSecs() - st) > 1.0/30.0
            sc_sample = device.receive_scene_video_frame();
            gaze_mapper.process_scene(sc_sample);
            st = GetSecs();
        end
        gz_sample = device.receive_gaze_datum();
        result = gaze_mapper.process_gaze(gz_sample);
